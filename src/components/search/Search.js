...
const {setLoadStudents,setSearchStudent,LoadEmployee,setLoadEmployee}=useGlobalContext()
//searchStudents
useEffect(()=>{
...
try {
  const filterdData=await axios.post(`http://localhost:5000/api/student/searchStudent`,{search:SearchInp},{withCredentials:true})
...
if(Search[1]==='SearchPage'){
  setSearchStudent(filterdData.data.student)
}  
} catch (error) {
console.log(error) 
}
...
if(Search[0]==='employee'){
  const filterdData=await axios.post(`http://localhost:5000/api/employee/searchemployee`,{search:SearchInp},{withCredentials:true})
