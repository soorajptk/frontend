...
const response=await axios.delete(`http://localhost:5000/api/student/${id}`,{withCredentials:true})
...
const response=await axios.get(`http://localhost:5000/api/student/approval/${id}`,{withCredentials:true})
console.log(response?.data)
setSuccess(response?.data?.msg)
}catch(err){
  setError(err.response)
    ...
    <article className={'SingleStudentContainer' }>
        ...
        <div className='singleStudentOptions'>
          {user?.role ==='admin' && <button className='deletebtn editbtnhide' onClick={()=>removeStudent(item._id)}>delete</button>}
           ...
           {want==='view' && <button  className='editbtn editbtnhide'>View</button>}
        </div>
    </article>
    )
}
