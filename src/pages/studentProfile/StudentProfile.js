...
function StudentProfile() {
    ...
    const imageUpload=async(e)=>{
        ...
        console.log(data)
        try {
        const response=await axios.post('http://localhost:5000/api/fileupload',data)
...
const handleSubmit=async(e)=>{
    e.preventDefault()
console.log(validate(loadStudentProfile))
}
    ...
    try {
    const response=await axios.get(`http://localhost:5000/api/student/${id}`,{withCredentials:true})
console.log()
  ...
  return (
    ...
    <section className='studentProfile'>
        ...
        <img className='profileimage' src={loadStudentProfile.photo} alt="studentPic" />
            ...
            {updateMode && <input type="text" value={loadStudentProfile.name} onChange={handleChange} />}
        ...
        <div className='profileAdditional'>
            ...
            {updateMode && <input onChange={handleChange} name='phone' value={loadStudentProfile.phone} type="text" />}
          <span><FontAwesomeIcon className='usericon' icon={faEnvelope}/>  </span>
            {!updateMode && <h4>{loadStudentProfile.email}</h4>}
            {updateMode && <input  value={loadStudentProfile.email} name='email' onChange={handleChange} type="text" />}
            <span><FontAwesomeIcon className='usericon' icon={faMapLocationDot}/>  </span>
            {!updateMode && <h4>{loadStudentProfile.Address}</h4>}
            {updateMode && <input value={loadStudentProfile.Address} name='Adrress' onChange={handleChange} type="text" />}
        ...
        <div className='records'>
            {
                ...
                <tr>
                    ...
                    <td><p className='recordsData'>{"100"}</p></td>
                </tr>
                </tbody>
            ...
            }
          {
           updateMode && <div>
            ...
            <input type="text" onChange={handleChange}  value={loadStudentProfile.HighestQualification} name="HighestQualification" />
            <label htmlFor="skills">skills</label>
            <input type="text" onChange={handleChange} value={loadStudentProfile.skillset} name="skillset" />
            <label htmlFor="emplyomentStatus">emplyomentStatus</label>
            <input type="text" onChange={handleChange} value={loadStudentProfile.emplyomentStatus} name="emplyomentStatus" />
            ...
            <input type="text" name="" onChange={handleChange} value={loadStudentProfile.TechnologyTraining}  />
            <label  htmlFor="qualification">Exit exam mark</label>
            <input disabled={user?.role ==='admin' || user?.role ==='employee'  ? false : true} type="text" onChange={handleChange}  value={"100"} name="TechnologyTraining"  />
            </div>
            
          }  
          <button type='submit' className='studentProfileUpdate' >update</button>
         </div>
        </article>
        <article>
            <p>hello</p>
