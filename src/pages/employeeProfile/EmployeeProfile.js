...
import {faUser,faCamera, faTrash, faUserEdit} from '@fortawesome/free-solid-svg-icons'
...
const loadSingleEmployee=async()=>{
    console.log(id)
try {
     const response=await axios.get(`http://localhost:5000/api/employee/${id}`,{withCredentials:true})
...
const uploadImage=async(e)=>{
    ...
    try {
        const upload=await axios.post('http://localhost:5000/api/fileupload',data)
...
const HandleSubmit=async(e)=>{
    ...
    try {
     const response=await axios.patch(`http://localhost:5000/api/employee/${UpdateUser?._id}`,UpdateUser)
